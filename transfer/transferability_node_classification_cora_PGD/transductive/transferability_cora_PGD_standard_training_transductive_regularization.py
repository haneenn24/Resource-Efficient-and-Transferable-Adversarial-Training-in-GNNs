...
# PGD Adversarial Attack with tqdm
def pgd_attack(data, model, epsilon=0.1, alpha=0.01, num_iter=20):
    original_data = data.x.clone().detach()  # Original data copy
    perturbed_data = original_data.clone().detach().requires_grad_(True)  # Enable gradient tracking

    for _ in tqdm(range(num_iter), desc="PGD Attack Iterations"):
        # Create a clone of data with the perturbed features
        data_adv = data.clone()
        data_adv.x = perturbed_data  # Use the perturbed data in the forward pass
        
        # Perform forward pass with the perturbed data
        output = model(data_adv)
        loss = F.nll_loss(output[data.train_mask], data.y[data.train_mask])  # Compute loss

        # Zero out gradients for the model and the perturbed data
        model.zero_grad()
        if perturbed_data.grad is not None:
            perturbed_data.grad.zero_()

        # Backpropagate to compute the gradients with respect to the perturbed data
        loss.backward()

        # Update the perturbed data using the gradients
        data_grad = perturbed_data.grad  # Compute gradient
        perturbed_data = perturbed_data + alpha * data_grad.sign()  # Apply gradient update

        # Clip the perturbations to the epsilon-ball around the original data
        perturbation = torch.clamp(perturbed_data - original_data, min=-epsilon, max=epsilon)
        perturbed_data = torch.clamp(original_data + perturbation, min=0, max=1).detach().requires_grad_(True)  # Reset for the next iteration
...
# Step 14: Generate Adversarial Examples using GAT with PGD
...
# Step 16: Generate Adversarial Examples using GPRGNN with PGD
