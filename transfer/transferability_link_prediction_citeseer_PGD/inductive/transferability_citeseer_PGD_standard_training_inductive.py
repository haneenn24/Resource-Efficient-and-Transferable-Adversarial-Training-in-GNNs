...
# PGD Adversarial Attack on Training Data with tqdm
def pgd_attack(data, model, epsilon=0.3, alpha=0.01, num_iter=20):
    original_data = data.x.clone()
    for _ in tqdm(range(num_iter), desc="PGD Attack on Training Data"):
...
# PGD Adversarial Attack on Test Data with tqdm
def pgd_attack_test(data, model, epsilon=0.3, alpha=0.01, num_iter=20):
    original_data = data.x.clone()
    for _ in tqdm(range(num_iter), desc="PGD Attack on Test Data"):
        data.x.requires_grad = True
        output = model(data)
