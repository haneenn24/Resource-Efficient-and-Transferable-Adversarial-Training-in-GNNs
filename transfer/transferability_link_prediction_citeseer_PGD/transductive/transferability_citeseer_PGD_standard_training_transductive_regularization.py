...
def pgd_attack(data, model, epsilon=0.1, alpha=0.01, num_iter=20):
    original_data = data.x.clone().detach()
    ...
    for _ in tqdm(range(num_iter), desc="PGD Attack Iterations"):
        ...
        perturbed_data = torch.clamp(original_data + perturbation, min=0, max=1).detach().requires_grad_(True)
...
# Generate Adversarial Examples using GCN with PGD
...
# Generate Adversarial Examples using GAT with PGD
...
# Generate Adversarial Examples using GPRGNN with PGD
