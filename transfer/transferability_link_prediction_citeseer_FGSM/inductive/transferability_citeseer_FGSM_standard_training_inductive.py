...
# FGSM Adversarial Attack on Training Data
def fgsm_attack(data, model, epsilon=0.3):
...
def fgsm_attack_test(data, model, epsilon=0.3):
    data.x.requires_grad = True
    output = model(data)
...
def evaluate_model(model, data, apply_adversarial=True, epsilon=0.3):
    # model.eval()
    # with torch.no_grad():
    # Clean evaluation
...
gcn_clean_acc, gcn_adv_acc = evaluate_model(gcn_model, test_data, apply_adversarial=True, epsilon=0.3)
gat_clean_acc_on_gcn_adv, gat_adv_acc_on_gcn_adv = evaluate_model(gat_model, test_data, apply_adversarial=True, epsilon=0.3)
gprgnn_clean_acc_on_gcn_adv, gprgnn_adv_acc_on_gcn_adv = evaluate_model(gprgnn_model, test_data, apply_adversarial=True, epsilon=0.3)

# Step 2: Generate adversarial examples using GAT on the test set
gat_clean_acc, gat_adv_acc = evaluate_model(gat_model, test_data, apply_adversarial=True, epsilon=0.3)
gcn_clean_acc_on_gat_adv, gcn_adv_acc_on_gat_adv = evaluate_model(gcn_model, test_data, apply_adversarial=True, epsilon=0.3)
gprgnn_clean_acc_on_gat_adv, gprgnn_adv_acc_on_gat_adv = evaluate_model(gprgnn_model, test_data, apply_adversarial=True, epsilon=0.3)

# Step 3: Generate adversarial examples using GPRGNN on the test set
gprgnn_clean_acc, gprgnn_adv_acc = evaluate_model(gprgnn_model, test_data, apply_adversarial=True, epsilon=0.3)
gcn_clean_acc_on_gprgnn_adv, gcn_adv_acc_on_gprgnn_adv = evaluate_model(gcn_model, test_data, apply_adversarial=True, epsilon=0.3)
gat_clean_acc_on_gprgnn_adv, gat_adv_acc_on_gprgnn_adv = evaluate_model(gat_model, test_data, apply_adversarial=True, epsilon=0.3)

# Print and log results
results = (
    f"Standard Training Results with FGSM Test Set Perturbation:\n"
    f"-----------------------------------------\n"
