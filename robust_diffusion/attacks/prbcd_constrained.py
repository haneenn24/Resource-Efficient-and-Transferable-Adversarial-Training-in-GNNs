...
from robust_diffusion.projections.projections import NodeL2Projection, GreedyKnapsackProjection


class LRBCD(SparseAttack):
    ...
    def __init__(self,
        ...
        self.local_budget = LRBCD.get_local_budget(local_factor, self.edge_index, self.edge_weight, self.n, round = 'floor')
        self.save_before_sampling = save_before_sampling
        #self.local_budget = self.n*torch.ones_like(self.local_budget)

        self.candidate_idx = self.get_candidate_nodes().view(-1)
        c = len(self.candidate_idx)

        if self.make_undirected:
            self.n_possible_edges = c* (c - 1) // 2
        else:
            self.n_possible_edges = c ** 2  # We filter self-loops later
    ...
    def _attack(self, n_perturbations, **kwargs):
        ...
        # init projection
        if self.projection_type == 'Greedy':
            self.projection = GreedyKnapsackProjection(self.local_budget, n_perturbations)
        elif self.projection_type == 'NodeL2':
            self.projection = NodeL2Projection(self.local_budget, n_perturbations)
        else:
            assert False, f'specified projection method ({self.projection_type}) not supported'
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            # in case model is unstable clean nans
            ...
            with torch.no_grad():
                ...
                # Projcetion to stay within glbal and local budget 
        ...
        if self.save_before_sampling:
            self.perturbed_edge_weight_befor_sampling = self.perturbed_edge_weight.detach().clone()
    ...
    def sample_final_edges(self, n_perturbations: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i in range(self.max_final_samples):
            if best_accuracy == float('Inf'):
                ...
                sampled_edges = self.projection.select(perturbed_edge_weight, self.modified_edge_index)
            ...
            if not self.projection.check_constraints(sampled_edges, self.modified_edge_index):
    ...
    def update_edge_weights(self, n_perturbations: int, epoch: int,
        ...
        """
        lr_factor = self.lr_factor # n_perturbations / self.n / 2 * self.lr_factor
    ...
    def sample_random_block(self, n_perturbations: int = 0):
        for _ in range(self.max_final_samples):
            ...
            if self.make_undirected:
                self.modified_edge_index = LRBCD.linear_to_triu_idx_only_candidates(self.n, self.current_search_space, self.candidate_idx)
            else:
                raise NotImplementedError('Only make_undirected=True supported')
                self.modified_edge_index = LRBCD.linear_to_full_idx(self.n, self.current_search_space)
    ...
    @staticmethod
    def linear_to_triu_idx_only_candidates(n: int, lin_idx: torch.Tensor, idx_candidates):
        m = len(idx_candidates)
        triu_small = LRBCD.linear_to_triu_idx(m, lin_idx)
        return idx_candidates[triu_small]
    ...
    def get_candidate_nodes(self):
        ''' returns all nodes with local budget >= 1'''
        candidates = (self.local_budget >= 1).nonzero().to(self.device)
        return candidates
    ...
    @staticmethod
    def get_local_budget(factor, edge_index, edge_weight, n, round = 'floor'):
        R = torch.sparse_coo_tensor(edge_index, edge_weight, (n,n)) # todo add graph size here
        neighbours_per_node = torch.sparse.sum(R, dim=1).to_dense()
        fractions = factor*neighbours_per_node
        if round == 'floor':
            local_budget = torch.floor(fractions)
        elif round == 'ceil':
            local_budget = torch.ceil(fractions)
        else:
            assert False
        return local_budget