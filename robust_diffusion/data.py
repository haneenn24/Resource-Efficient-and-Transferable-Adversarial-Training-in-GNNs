...
@typechecked
def prep_karate(device: Union[int, str, torch.device] = 0,
              make_undirected: bool = True):
    assert make_undirected == True , "undirected not implemented for karate club"
    karate = KarateClub()
    attr = karate.data.x.to(device)
    labels = karate.data.y.to(device)
    edge_index = karate.data.edge_index.to(device)
    edge_weight = torch.ones(edge_index.shape[1]).to(device)
    adj = SparseTensor(row=edge_index[0], col=edge_index[1], value=edge_weight, sparse_sizes=(edge_index.max()+1, edge_index.max()+1)).to(device)
    return attr, adj, labels
...
def prep_graph(name: str,
               ...
               seed: int=0) -> Tuple[TensorType["num_nodes", "num_features"],
                                     SparseTensor,
                                     TensorType["num_nodes"],
                                     Optional[np.ndarray],  # Return splits as arrays
                                     Optional[np.ndarray],
                                     Optional[np.ndarray]]:
    """Prepares and normalizes the desired dataset"""
    ...
    if name in ['cora_ml', 'citeseer', 'pubmed']:
        ...
        idx_train, idx_val, idx_test = split_cora(labels)  # Generate splits for Cora
    ...
    # Other datasets like ogbn, sbm, etc.
    # ...
    
    if return_original_split or split is not None:
        return attr, adj, labels, split['train'], split['valid'], split['test']

    return attr, adj, labels, idx_train, idx_val, idx_test  # Return splits

def split_cora(labels, num_train_per_class=20, num_val=500, num_test=1000, seed=0):
    """Splits the Cora dataset into train, validation, and test sets."""
    np.random.seed(seed)
    
    # Move labels to CPU if they're on the GPU
    ...
    for i in range(labels.max() + 1):
        ...
        idx_train.append(class_idx[:num_train_per_class])
    idx_train = np.concatenate(idx_train)

    idx_val = np.setdiff1d(idx, idx_train)[:num_val]
    idx_test = np.setdiff1d(idx, np.concatenate([idx_train, idx_val]))[:num_test]
...
def filter_data_for_idx(attr, adj, labels, idx):
    ...
    row = mapping_proj[row]
    col = mapping_proj[col]

    # map attr
    attr = attr[idx]
    labels = labels[idx]

    return attr, SparseTensor(row=row.to(0), col=col.to(0), value=edge_weight.to(0), sparse_sizes=(n_idx, n_idx)), labels, mapping_proj.cpu().numpy()
...
def split_squirrel(labels, train_size=0.6, val_size=0.2, test_size=0.2, seed=None):
    labels = labels.copy()
    ...
    idx_train_and_val, idx_test = train_test_split(idx,
                                                   ...
                                                   test_size=test_size,
                                                   stratify=labels)
    labels = labels[idx_train_and_val]
    idx_train, idx_val = train_test_split(idx_train_and_val,
                                          random_state=seed,
                                          train_size=(train_size / (train_size + val_size)),
                                          test_size=(val_size / (train_size + val_size)),
